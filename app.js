...
userOverlay?.classList.add('hidden'); userOverlay?.classList.remove('show');
userModal?.classList.add('hidden');   userModal?.classList.remove('show');

let __onUserDone = null;
function openUserModal(initialName = 'Khách', onDone){
...
// Enter/Esc
document.addEventListener('keydown', (e)=>{
  if (userModal?.classList.contains('hidden')) return;
  ...
  openUserModal(USER_ID || 'Khách', (name)=>{
    USER_ID = name;
    localStorage.setItem('quizUserId', USER_ID);
    userChip && (userChip.textContent = `👤 ${USER_ID}`);
  });
